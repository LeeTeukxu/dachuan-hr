...
import com.tianye.hrsystem.modules.insurance.entity.*;
import com.tianye.hrsystem.modules.insurance.mapper.HrmInsuranceMonthEmpRecordMapper;
import com.tianye.hrsystem.modules.insurance.mapper.HrmInsuranceSchemeMapper;
 ...
 */
@Service
public class HrmInsuranceMonthEmpRecordService extends BaseServiceImpl<HrmInsuranceMonthEmpRecordMapper, HrmInsuranceMonthEmpRecord> {
    @Autowired
    private HrmInsuranceMonthEmpRecordMapper monthEmpRecordMapper;

    @Autowired
    private HrmInsuranceSchemeService schemeService;

    @Autowired
    private IHrmEmployeeService employeeService;

    @Autowired
    private HrmEmployeeSocialSecurityService socialSecurityService;

    @Autowired
    private HrmInsuranceMonthEmpProjectRecordService empProjectRecordService;

    @Autowired
    private HrmInsuranceProjectService projectService;
    ...
    public Page<QueryEmpInsuranceMonthVO> queryEmpInsuranceMonth(QueryEmpInsuranceMonthBO queryEmpInsuranceMonthBO) {
        Page<QueryEmpInsuranceMonthVO> page = monthEmpRecordMapper.queryEmpInsuranceMonth(queryEmpInsuranceMonthBO.parse(), queryEmpInsuranceMonthBO);
        return page;
    }
    ...
    public EmpInsuranceByIdVO queryById(String iempRecordId) {
        ...
        empInsuranceByIdVO.setCity(String.valueOf(scheme.getCity()));
...
//            }
    ...
    public List<OperationLog> addInsuranceEmp(AddInsuranceEmpBO addInsuranceEmpBO) {
        Long iRecordId = addInsuranceEmpBO.getRecordId();
        HrmInsuranceMonthRecord monthRecord = monthRecordService.getById(iRecordId);

        List<OperationLog> operationLogList = new ArrayList<>();
        if (addInsuranceEmpBO.getEmployeeIds().size() > 0) {
            List<HrmInsuranceMonthEmpRecord> lists = new ArrayList<>();
            for (Long employeeId : addInsuranceEmpBO.getEmployeeIds()) {
                ...
                if (!insuranceMonthEmpRecordOpt.isPresent()) {
                    HrmInsuranceMonthEmpRecord records = new HrmInsuranceMonthEmpRecord();
                    records.setIRecordId(iRecordId);
                    records.setEmployeeId(employeeId);
                    records.setYear(monthRecord.getYear());
                    records.setMonth(monthRecord.getMonth());
                    records.setPersonalInsuranceAmount(new BigDecimal("0"));
                    records.setPersonalProvidentFundAmount(new BigDecimal("0"));
                    records.setCorporateInsuranceAmount(new BigDecimal("0"));
                    records.setCorporateProvidentFundAmount(new BigDecimal("0"));
                    records.setStatus(1);
                    lists.add(records);
                }
            }
    ...
    public List<OperationLog> batchUpdateInsuranceProject(UpdateInsuranceProjectBO updateInsuranceProjectBO) {
        List<OperationLog> operationLogList = new ArrayList<>();
        updateInsuranceProjectBO.setIEmpRecordIds(updateInsuranceProjectBO.getTempIEmpRecordIds());
