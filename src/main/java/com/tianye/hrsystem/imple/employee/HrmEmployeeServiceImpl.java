...
//        messageService.save(adminMessage);
            ...
            if(StringUtils.isEmpty(beComeTime)==false){
                ...
                try {
                    ...
                    if(emp.getCompanyAgeStartTime()==null){
                        ...
                        emp.setCompanyAgeStartTime(dBeCome);
                        emp.setCompanyAge(Math.toIntExact(DateUtil.betweenYear(simple.parse(emp.getCompanyAgeStartTime().toString()),new Date(),false)));
                    }
                }
                catch(Exception z){
                   // z.printStackTrace();
    ...
    public void importEmployee(MultipartFile multipartFile) throws Exception {
        if (multipartFile != null) {
            ...
            List<com.tianye.hrsystem.model.HrmEmployee> list = new ArrayList<>();
            List<List<Object>> read = reader.read();
            List<com.tianye.hrsystem.model.HrmDept> listDepts = deptRep.findAll();
            List<com.tianye.hrsystem.model.HrmEmployee> listEmployees = employeeRep.findAll();
            for (int i = TWO; i < read.size(); i++) {
                com.tianye.hrsystem.model.HrmEmployee employee = new com.tianye.hrsystem.model.HrmEmployee();
                List<Object> row = read.get(i);
                Optional<com.tianye.hrsystem.model.HrmEmployee> findOne = listEmployees.stream().filter(f -> f.getIdNumber().equals(row.get(3).toString())).findFirst();
                if (findOne.isPresent()) {
                    BeanUtils.copyProperties(findOne.get(), employee);
                    employee.setEmployeeName(row.get(0).toString());
                    employee.setMobile(row.get(1).toString());
                    employee.setIdType(getidType(row.get(2).toString()));
                    employee.setIdNumber(row.get(3).toString());
                    employee.setSex(getSex(row.get(4).toString()));
                    employee.setFullAttendance(getFullAttendance(row.get(5).toString()));
                    employee.setExpandProduction(getExpandProduction(row.get(6).toString()));
                    employee.setEntryTime(LocalDate.parse(row.get(7).toString(), formatter));
                    employee.setEmploymentForms(getEmploymentForms(row.get(8).toString()));
                    listDepts.stream().forEach(f -> {
                        if (f.getName().equals(row.get(9).toString())) {
                            employee.setDeptId(f.getDeptId());
                        }
                    });
                    employee.setProbation(getProbation(row.get(10).toString()));
                }else {
                    employee.setEmployeeName(row.get(0).toString());
                    employee.setMobile(row.get(1).toString());
                    employee.setIdType(getidType(row.get(2).toString()));
                    ...
                    employee.setCreateUserId(Info.getUserIdValueL());
                    employee.setJobNumber(Long.toString(System.currentTimeMillis()));

                    String probation = row.get(10).toString();
                    String employmentForms = row.get(8).toString();
                    if (!employmentForms.equals("") && !probation.equals("")) {
                        if (employmentForms.equals("正式") && !probation.equals("无试用期")) {
                            LocalDateTime dateTime = LocalDateTimeUtil.offset(employee.getEntryTime().atStartOfDay(), getProbation(probation), ChronoUnit.MONTHS);
                            if (dateTime.isAfter(LocalDateTimeUtil.beginOfDay(LocalDateTime.now()))) {
                                employee.setStatus(EmployeeStatusEnum.TRY_OUT.getValue());
                            } else {
                                employee.setStatus(EmployeeStatusEnum.OFFICIAL.getValue());
                            }
                            employee.setBecomeTime(dateTime);
                        }else {
                            employee.setStatus(EmployeeStatusEnum.OFFICIAL.getValue());
                            employee.setBecomeTime(LocalDateTime.parse(row.get(7).toString()));
                        }
                    }else {
                        employee.setStatus(EmployeeStatusEnum.OFFICIAL.getValue());
                        employee.setBecomeTime(LocalDateTime.parse(row.get(7).toString(), formatter));
                    }
                }
                list.add(employee);
            }
//            employeeRep.saveAll(list);
        }
    }
