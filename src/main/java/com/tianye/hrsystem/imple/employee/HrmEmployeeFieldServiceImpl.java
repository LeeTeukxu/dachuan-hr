...
import com.tianye.hrsystem.mapper.HrmEmployeeFieldMapper;
...
import com.tianye.hrsystem.service.IHrmConfigService;
    ...
    public List<EmployeeHeadFieldVO> queryListHeads() {
        ...
        if (!exists) {
            ...
            List<HrmEmployeeFieldConfig> fieldConfigList = isHeadFieldIds.stream().map(fieldId -> {
                ...
                fieldConfig.setUserId(user.getUserIdValueL());
        ...
        List<EmployeeHeadFieldVO> listHeads = employeeFieldMapper.queryListHeads(user.getUserIdValueL());
            ...
            if (fieldData.getType().equals(FieldTypeEnum.CHECKBOX.getValue())) {
                if (StrUtil.isNotEmpty(fieldData.getFieldValue())) {
                    ...
                    if (fieldData.getFieldValue().contains(contains)) {
                        ...
                        List<String> optionList = Arrays.stream(fieldData.getFieldValue().split(",")).collect(toList());
...
//        if (depth < 0) {
//            throw new Exception(SystemCodeEnum.SYSTEM_ERROR);
//        }
