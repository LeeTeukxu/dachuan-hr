...
public class HolidayDataServiceImplTest {
    ...
    @InjectMocks
    private HolidayDataServiceImpl service;
    ...
    public void sync_shouldReuseInjectedUsersAndSkipDuplicateMappings() throws Exception {
        ...
        verify(report, times(1)).UpdateHolidayReportQuick(eq("u1"), eq(1001L), eq(begin), eq(end));
        verify(report, times(1)).UpdateHolidayReportQuick(eq("u2"), eq(1002L), eq(begin), eq(end));
        verify(report, never()).UpdateHolidayReportQuick(eq("u3"), eq(1003L), eq(begin), eq(end));
