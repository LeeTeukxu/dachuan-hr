...
- 2026-09-02 通知中心功能（原始版）：新增系统级通知中心，显示考勤同步、审批获取、社保计算等任务的 running 和完成状态。后端新增 `admin_message` 表 `link_url` 字段、`AdminMessageEnum` 枚举 201-206、`AdminMessageMapper`/`AdminMessageServiceImpl`/`AdminMessageController`；三个 Service 在任务开始和完成时写入通知；前端新增 `NotificationCenter.vue` 组件（铃铛图标、99+徽章、下拉列表）和 `NotificationList.vue` 页面，集成到 `TopHeader.vue`。修复前后端接口不匹配问题（参数名 `id`→`messageId`、数据格式 `messages`→`records`、分页参数适配）。后端 `/adminMessage/list` 新增 `isRead` 可选参数支持筛选；考勤同步通知 `linkUrl` 从 `/hrm/attendance/records` 修正为 `/hrm/attendance/scheduling`。
...
- 2026-09-02 数据源稳定性修复：`DynamicDataSource` 新增 `HikariDataSource.isClosed()` 检测，evictor 驱逐关闭池后自动从 `OX` 移除并重建；`CompanyDataSourceProvider` 新增活跃租户集合（`markActive/markInactive`），后台任务期间 evictor 跳过该租户防驱逐；`AttendanceSyncTaskLauncher.submit()` 在任务开始/结束时注册/注销活跃租户；`tbattendanceapprove.userId` 与 `tbattendancerecord.userId` 从 `varchar(20)` 扩大到 `varchar(100)`，修复钉钉 userId 超长导致的 Data truncation。
